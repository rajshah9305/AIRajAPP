      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-primary-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-lg font-bold bg-gradient-to-r from-primary-600 to-primary-700 bg-clip-text text-transparent">RAJ AI APP BUILDER</div>
              <div className="text-xs text-gray-600">Generate React apps with AI</div>
            </div>
          </div>
          <Link href="/builder" className="px-3 py-1.5 rounded-lg bg-gradient-to-r from-primary-500 to-primary-600 text-white text-sm shadow hover:shadow-md transition-shadow">
            Start building
          </Link>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
          <div className="lg:col-span-2 bg-white border border-gray-200 rounded-2xl shadow-2xl p-6 sm:p-8">
            <div className="w-fit mb-4 sm:mb-6 px-2 py-1 rounded-full bg-primary-100 text-primary-700 text-xs border border-primary-200">Personal AI project</div>
            ...
            <p className="mt-4 sm:mt-6 text-base sm:text-lg text-gray-600 leading-relaxed">
              Describe what you want. Watch code stream in real-time with a live preview. Iterate using a chat input anchored at the bottom. No auth, no mockups — just working code.
            </p>
            <div className="mt-6 sm:mt-8 flex flex-wrap items-center gap-3">
              <Link href="/builder" className="inline-flex items-center gap-2 px-4 sm:px-5 py-2.5 rounded-lg bg-gradient-to-r from-primary-500 to-primary-600 text-white font-semibold shadow hover:shadow-lg transition-all">
                Start building now
                <ArrowRight className="w-4 h-4" />
              </Link>
              <a href="https://github.com/rajshah9305/AIRajAPP" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-4 sm:px-5 py-2.5 rounded-lg bg-white border border-gray-200 text-gray-700 font-medium shadow-sm hover:shadow transition-all">
                View on GitHub
              </a>
            </div>
            ...
            <div className="mt-6 sm:mt-8">
              ...
              <div className="flex flex-wrap gap-2">
                {EXAMPLE_PROMPTS.map((p) => (
                  <Link key={p} href={`/builder?prompt=${encodeURIComponent(p)}`} className="px-3 py-1.5 rounded-full text-sm bg-gray-100 hover:bg-gray-200 text-gray-700 transition-colors">
                    {p}
                  </Link>
                ))}
              </div>
