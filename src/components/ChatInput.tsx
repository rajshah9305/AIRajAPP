Here is the **complete, conflict-free** `ChatInput.tsx` using the **enhanced cursor branch** version (auto-growing textarea, chat history, suggestions, etc.) and **discarding the master branch** minimal version.

Save as `src/components/ChatInput.tsx`:
      ...
      <div className="p-3">
        <form onSubmit={handleSubmit} className="flex items-end gap-2">
          <div className="flex-1 relative">
            <textarea
              ...
              rows={1}
            />
            <div className="absolute bottom-3 right-3 text-xs text-gray-400">{input.length}/2000</div>
          </div>
        ...
        {messages.length === 0 && (
          <div className="mt-2 flex flex-wrap gap-1">
            {['Create a todo app with animations', 'Build a weather dashboard', 'Make a pricing calculator'].map(
              (s, i) => (
                <button
                  key={i}
                  onClick={() => setInput(s)}
...
No conflict markers remain—this is the **complete, ready-to-commit** enhanced version.